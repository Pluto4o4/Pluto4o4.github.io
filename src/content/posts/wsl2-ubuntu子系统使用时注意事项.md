...
description: "WSL2 Ubuntu子系统配置指南,包括zsh配置、代理设置和SSH连接等"
...
```bash
# 我使用的是powerlevel10k_lean比较简洁
eval "$(oh-my-posh --init --shell zsh --config ~/.poshthemes/主题名.omp.json)"
```
...
export PATH=`echo $PATH | sed 's/:\/mnt\/d\/[^:]*//g'` # 替换d盘的环境变量
export PATH=`echo $PATH | sed 's/:\/mnt\/e\/[^:]*//g'` # 替换e盘的环境变量
```
